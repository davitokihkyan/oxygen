<script setup>
    ...
    watch(() => props.pageProps, (pageProps) => {
        if (pageProps.alert) {
...
<template>
    <div v-if="showInfoMessage" class="fixed bottom-5 right-5">
        <div class="flex items-center w-full bottom-5 left-5 max-w-xs p-4 mb-4 text-gray-500 bg-white rounded-lg shadow">
            ...
            <div class="ml-3 text-sm font-normal">{{ infoMessage }}</div>
            <button type="button" class="ml-auto -mx-1.5 -my-1.5 bg-white text-gray-400 hover:text-gray-900 rounded-lg p-1.5 hover:bg-gray-100 inline-flex items-center justify-center h-8 w-8">
                <span @click="closeAlert" class="sr-only">Close</span>
                <svg @click="closeAlert" class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6"/>
                </svg>
            </button>
        </div>
    </div>
    <div v-if="showErrorMessage" class="fixed bottom-5 right-5">
